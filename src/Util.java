						...
						if (intraECUBusNode != null) {
							...
							for (int j=0; j<intraECUBusChildren.getLength(); j++) {
								...
								if (_iebcoresNode.getNodeType() == 1) {
									NodeList _iebcoresChildren = _iebcoresNode.getChildNodes();
									List<Core> _cores = new ArrayList<Core>();
									for (int k=0; k<_iebcoresChildren.getLength(); k++) {
										Node _iebcoreNode = _iebcoresChildren.item(k);
			...
			if (interecubusNode != null) {
				...
				for (int i=0; i<interecubusChildren.getLength(); i++) {
					...
					if (_iebecusNode.getNodeType() == 1) {
						...
						List<ECU> ecus = new ArrayList<ECU>();
